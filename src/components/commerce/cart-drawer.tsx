        ...
        <>
          ...
          >
            <div className="flex h-16 items-center justify-between border-b border-border px-5">
              ...
              <button type="button" aria-label="Fermer le panier" className="size-10 text-2xl" onClick={() => setCartOpen(false)}>×</button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 min-[390px]:p-5">
              {cartItems.length === 0 ? (
                <p className="py-16 text-center text-sm text-charcoal">Votre panier est vide.</p>
              ) : (
                        ...
                        <div>
                          ...
                          <div className="mt-4 flex items-center justify-between">
                            <div className="flex border border-border">
                              <button className="size-8" onClick={() => changeQuantity(item.key, item.quantity - 1)} aria-label="Réduire la quantité">−</button>
                              <span className="flex w-8 items-center justify-center text-xs">{item.quantity}</span>
                              <button className="size-8" onClick={() => changeQuantity(item.key, item.quantity + 1)} aria-label="Augmenter la quantité">+</button>
                            </div>
            ...
            <div className="shrink-0 border-t border-border p-4 pb-[max(1rem,env(safe-area-inset-bottom))] min-[390px]:p-5">
              <div className="flex justify-between text-sm"><span>Sous-total</span><strong>{formatTnd(subtotal)}</strong></div>
              ...
              <Button disabled className="mt-5 w-full">Paiement indisponible dans la MVP</Button>
              <button type="button" onClick={() => setCartOpen(false)} className="mt-4 w-full text-xs uppercase tracking-[0.14em] underline">Poursuivre vos achats</button>
            </div>
          </motion.aside>
        </>
      )}
